...
const DashboardContent = () => {
    ...
    return (
        <div className="w-full h-full">
            <div className="mb-8">
                <h2 className="text-[30px] font-bold">Dashboard</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* Doctors Card */}
                <div className="group relative overflow-hidden transition-transform duration-300 hover:scale-105 shadow-md rounded-2xl">
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-indigo-600/20 rounded-2xl blur-xl z-0"></div>
                    <div className="relative bg-white/90 backdrop-blur-sm rounded-2xl p-8 h-64 shadow-xl border border-white/20 z-10">
                        <div className="flex items-start justify-between h-full">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600">Active practitioners</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 mb-4">
                                    <Activity className="w-4 h-4 text-blue-500" />
                                    <span className="text-sm text-blue-600 font-semibold">All active</span>
                                </div>
                            </div>
                            <div className="flex flex-col items-end justify-center h-full">
                                <div className="text-6xl font-bold text-gray-700 mb-2">
                                    {isLoading ? (
                                        ...
                                        <CountUpAnimation target={doctorsNo} isVisible={!isLoading} />
                                    )}
                                </div>
                                <div className="flex gap-1">
                                    {[...Array(5)].map((_, i) => (
                                        <div
                                            key={i}
                                            className={`w-2 h-8 rounded-full transition-all duration-500 ${i < doctorsNo ? 'bg-blue-500' : 'bg-gray-200'}`}
                                        />
                                    ))}
                                </div>
                            </div>
